(function () {
	...
	function getElements(trigger) {
		...
		const popup = document
			...
			?.querySelector('[data-tui-timepicker-popup]');
		if (!popup) return null;
		...
		return {
			...
			hiddenInput:
				...
				trigger.parentElement?.querySelector(
					'[data-tui-timepicker-hidden-input]',
				),
		};
	}
	...
	function getState(trigger) {
		return {
			...
			minute: trigger.dataset.tuiTimepickerCurrentMinute
				...
				: null,
			use12Hours:
				trigger.getAttribute('data-tui-timepicker-use12hours') === 'true',
			...
			placeholder:
				trigger.getAttribute('data-tui-timepicker-placeholder') ||
				'Select time',
		};
	}
	...
	function updateSelections(elements, state) {
		// Update hour buttons
		elements.hourList
			.querySelectorAll('[data-tui-timepicker-hour]')
			.forEach(btn => {
		...
		elements.minuteList
			.querySelectorAll('[data-tui-timepicker-minute]')
			.forEach(btn => {
	...
	document.addEventListener('click', e => {
		...
		if (target.matches('[data-tui-timepicker-minute]') && !target.disabled) {
			...
			const minute = parseInt(
				target.getAttribute('data-tui-timepicker-minute'),
			);
	...
	document.addEventListener('reset', e => {
		...
		e.target
			.querySelectorAll('[data-tui-timepicker="true"]')
			.forEach(trigger => {
				setState(trigger, null, null);
				const elements = getElements(trigger);
				if (elements?.hiddenInput) {
	...
	new MutationObserver(initializeTimePickers).observe(document.body, {
		childList: true,
		subtree: true,
	});
})();
