(function () {
	...
	function syncSelectionsFromValue(trigger) {
		...
		const isMultiple =
			trigger.getAttribute('data-tui-selectbox-multiple') === 'true';
	...
	function updateDisplayValue(trigger) {
		...
		const isMultiple =
			trigger.getAttribute('data-tui-selectbox-multiple') === 'true';
		const showPills =
			trigger.getAttribute('data-tui-selectbox-show-pills') === 'true';
		const placeholder =
			valueEl.getAttribute('data-tui-selectbox-placeholder') || 'Select...';
		...
		if (isMultiple) {
			if (showPills) {
				...
				Array.from(selectedItems).forEach(selectedItem => {
					...
					removeBtn.className =
						'ml-0.5 hover:text-destructive focus:outline-none';
				...
				requestAnimationFrame(() => {
					...
					if (contentWidth > containerWidth - 10 && selectedItems.length > 3) {
						const countText =
							...
							'{n} items selected';
						valueEl.textContent = countText.replace(
							'{n}',
	...
	function filterItems(searchInput) {
		...
		content.querySelectorAll('.select-item').forEach(item => {
			const text =
				item.querySelector('.select-item-text')?.textContent.toLowerCase() ||
				'';
			const value =
				item.getAttribute('data-tui-selectbox-value')?.toLowerCase() || '';
			const visible =
				!searchTerm || text.includes(searchTerm) || value.includes(searchTerm);
	...
	document.addEventListener('click', e => {
		...
		if (e.target.matches('[data-tui-selectbox-pill-remove]')) {
			...
			const item = content?.querySelector(
				`.select-item[data-tui-selectbox-value="${value}"]`
			);
	...
	document.addEventListener('keydown', e => {
		...
		if (activeElement?.matches('button.select-trigger')) {
			if (e.key.length === 1 || e.key === 'Backspace') {
				e.preventDefault();
				const contentId = activeElement.getAttribute(
				...
				setTimeout(() => {
					const searchInput = document
						...
						?.querySelector('[data-tui-selectbox-search]');
					if (searchInput) {
						searchInput.focus();
		...
		if (content?.querySelector('.select-item')) {
			if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
				...
				if (activeElement?.matches('.select-item')) {
					...
					const trigger = document.querySelector(
						`[data-tui-selectbox-content-id="${content.id}"]`
					);
					setTimeout(() => trigger?.focus(), 50);
				}
			}
