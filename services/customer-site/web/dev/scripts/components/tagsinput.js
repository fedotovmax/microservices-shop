(function () {
	...
	function addTag(container, value) {
		...
		const tagChip = createTagChip(
			tagValue,
			textInput?.hasAttribute('disabled'),
		);
	...
	document.addEventListener('keydown', e => {
		...
		if (e.key === 'Enter' || e.key === ',') {
			...
			const removeButton = lastChip?.querySelector(
				'[data-tui-tagsinput-remove]',
			);
	...
	document.addEventListener('reset', e => {
		...
		e.target.querySelectorAll('[data-tui-tagsinput]').forEach(container => {
			container
				.querySelectorAll('[data-tui-tagsinput-chip]')
				.forEach(chip => chip.remove());
			container
				.querySelectorAll(
					'[data-tui-tagsinput-hidden-inputs] input[type="hidden"]',
				)
