(function () {
	...
	document.addEventListener('input', e => {
		const slider = e.target.closest(
			'input[type="range"][data-tui-slider-input]'
		);
		if (!slider || !slider.id) return;

		document
			.querySelectorAll(
				`[data-tui-slider-value][data-tui-slider-value-for="${slider.id}"]`
			)
			.forEach(el => {
	...
	new MutationObserver(() => {
		document
			.querySelectorAll('input[type="range"][data-tui-slider-input]')
			.forEach(slider => {
				if (!slider.id) return;
