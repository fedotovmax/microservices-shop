(function () {
	...
	function getConfig(ratingElement) {
		return {
			value:
				parseFloat(
					ratingElement.getAttribute('data-tui-rating-initial-value'),
				) || 0,
			precision:
				parseFloat(ratingElement.getAttribute('data-tui-rating-precision')) ||
				1,
			readonly:
				ratingElement.getAttribute('data-tui-rating-readonly') === 'true',
			name: ratingElement.getAttribute('data-tui-rating-name') || '',
			onlyInteger:
				ratingElement.getAttribute('data-tui-rating-onlyinteger') === 'true',
		};
	}

	function getCurrentValue(ratingElement) {
		return (
			parseFloat(ratingElement.getAttribute('data-tui-rating-current')) ||
			parseFloat(ratingElement.getAttribute('data-tui-rating-initial-value')) ||
			0
		);
	}
	...
	function setCurrentValue(ratingElement, value) {
		...
		const hiddenInput = ratingElement.querySelector(
	...
	function updateItemStyles(ratingElement, displayValue) {
		...
		ratingElement.querySelectorAll('[data-tui-rating-item]').forEach(item => {
			const itemValue = parseInt(
				item.getAttribute('data-tui-rating-value'),
				10,
			);
			if (isNaN(itemValue)) return;

			const foreground = item.querySelector(
	...
	document.addEventListener('reset', e => {
		...
		e.target
			.querySelectorAll('[data-tui-rating-component]')
		...
		document
			...
			.forEach(ratingElement => {
				...
				if (!ratingElement.hasAttribute('data-tui-rating-current')) {
					...
					setCurrentValue(
						ratingElement,
						Math.round(value / config.precision) * config.precision,
					);
				}
				...
				if (config.readonly) {
					...
					ratingElement
						.querySelectorAll('[data-tui-rating-item]')
						.forEach(item => {
	...
	new MutationObserver(initializeRatings).observe(document.body, {
		childList: true,
		subtree: true,
	});
})();
