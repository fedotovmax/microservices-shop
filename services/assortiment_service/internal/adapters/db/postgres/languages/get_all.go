...
const findAllQuery = "select code, is_default, is_active from languages;"

func (p *postgres) FindAll(ctx context.Context) ([]domain.Language, error) {
	const op = "adapters.db.postgres.languages.FindAll"

	tx := p.ex.ExtractTx(ctx)

	rows, err := tx.Query(ctx, findAllQuery)
