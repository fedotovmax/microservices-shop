...
func (u *usecases) handleUserBypass(ctx context.Context, user *domain.SessionsUser, params bypassParams) error {
	...
	if !user.HasBypass() {
		userActiveSessions, err := u.storage.sessions.FindUserSessions(ctx, user.Info.UID)
	...
	err := u.storage.sessions.RemoveSecurityBlock(ctx, db.SecurityTableBypass, user.Info.UID)
